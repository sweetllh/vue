<template>
	<div class="content">
	    ...
	    <ul>
	        ...
	        <li>
	        	<input type="text" value="" v-model="passw"/>
	            <span>密码</span>
	        </li>
	    </ul>
	    <div class="footbox">
	        <input type="button" value="登 录" class="login-btn" @click="jump()"/>
	...
	export default{
		data(){
				return {
					...
					auser:window.localStorage.getItem('locname').split('/'),
					apass:window.localStorage.getItem('locpsw').split('/')
				}
		},
		methods:{
			check(){
					for(var i=0;i<this.auser.length;i++){
						if(this.username==this.auser[i]&&this.passw==this.apass[i]){
							return true;
							break
						}
					}
