<template>
	<div class="content">
	    ...
	    <div class="footbox">
	        <input type="button" value="注 册" class="login-btn"  @click="jump()"/>
		...
		methods:{
			...
			jump(){
				if(this.loc.locname){
						...
						this.loc.locname+='/'+this.username;
				}
				else{
					this.loc.locname=this.passw;
				};
					
				if(this.loc.locpsw){
					this.loc.locpsw+='/'+this.username;
				}
				else{
					this.loc.locpsw=this.passw;
				};
